...
export default defineConfig({
  ...
  test: {
    ...
    // dedupe만으로는 react-router-dom이 여전히 externalize(Node 네이티브 import)되어
    // Vite 그래프를 안 타 dedupe가 적용되지 않는 문제가 있었다 — inline으로 강제 처리해
    // useLocation/useNavigate가 항상 같은 모듈 인스턴스를 참조하게 함.
    server: {
      deps: {
        inline: true,
      },
